...
2. Once the Twitch account has been linked, the API server will validate the authorization response from the Ftv-app Twitch app and issue a otp token for validation purposes.

3. The API server will then redirect the user back to Fansly with a jwt (JSON Web Token).
